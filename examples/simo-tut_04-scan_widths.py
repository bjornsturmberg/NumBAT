...
for i_w, width_obj in enumerate(width_objs):
    ...
    n_eff_sim = round(np.real(sim_EM.Eig_value[0]*((wl_nm*1e-9)/(2.*np.pi))), 4)
    ...
    k_AC = 2*np.real(sim_EM.Eig_value[0])
...
ax.set_zlabel('Gain 1/(Wm)')
ax.set_zlim3d(0,1500)
plt.savefig('gain_spectra_all.pdf')
plt.close()
