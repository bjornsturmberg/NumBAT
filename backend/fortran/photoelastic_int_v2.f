...
C
      subroutine photoelastic_int_v2 (lambda, nval_EM, nval_AC, ival1,
...
c         The components (E_x,E_y) of the mode ival
          do j=1,2
            E_field_el1(j,i) = soln_EM(j,i,ival1,iel)
            E_field_el2(j,i) = soln_EM(j,i,ival2,iel)
            U_field_el(j,i) = soln_AC(j,i,ival3,iel)
          enddo
c         The component E_z of the mode ival. The FEM code uses the scaling:
c         E_z = ii * beta1 * \hat{E}_z
          j=3
            E_field_el1(j,i) = ii * beta1 *soln_EM(j,i,ival1,iel)
            E_field_el2(j,i) = ii * beta2 * soln_EM(j,i,ival2,iel)
            U_field_el(j,i) = soln_AC(j,i,ival3,iel)
        enddo
