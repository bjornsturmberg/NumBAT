...
C
     ...
     *  type_el, x, beta1,
     *  soln_k1, soln_k2, overlap, overlap_file, PrintAll,
     *  pair_warning, k_0)
c
      implicit none
      ...
      complex*16 beta1
C      complex*16 overlap(nval,nval)
      complex*16, dimension(nval,nval) :: overlap
...
C     Mode ordering
      ...
      integer*8 compcount, elcount, nodecount, j2
...
c       Scalar product
        ...
        do i=1,2*nnodes
          ...
          overlap(ival,jval) = overlap(ival,jval)
     *          + z_tmp1
        enddo
      enddo
C
