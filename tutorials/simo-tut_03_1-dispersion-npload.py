...
# general estimated effective index of fundamental guided mode.
...
def ac_mode_freqs(k_ac):
    ...
    return [mode_freqs, k_ac]


# now we utilise multi-core calculations to perform parrallel simulations and speed up the simulation
test_name = 'dispersion_test.pdf'
...
for i_w, ac_object in enumerate(pooled_mode_freqs):
    sim_freqs = ac_object[0]
    sim_k_ac = ac_object[1]
...
for idx in range(plot_range):
    ...
    plt.plot(acoustic_ks/k_AC, freq_slice, 'k')
